...
def test_route_handlers_directly():
    print("\n--- 6. Testing Funding Route Handlers ---")
    db = SessionLocal()
    try:
        from app.routes.funding import get_all_normalized_opportunities, get_normalized_opportunity_by_id, check_duplicate_opportunities
        
        opps = get_all_normalized_opportunities(active_only=True, db=db)
        print(f"✓ Route handler get_all_normalized_opportunities returned {len(opps)} active opportunities.")
        assert len(opps) > 0
        
        single_opp = get_normalized_opportunity_by_id(funding_id=1, db=db)
        title_str = single_opp["title"] if isinstance(single_opp, dict) else single_opp.title
        opp_id = single_opp["id"] if isinstance(single_opp, dict) else single_opp.id
        print(f"✓ Route handler get_normalized_opportunity_by_id(1) returned '{title_str}'.")
        assert opp_id == 1
        
        dup_res = check_duplicate_opportunities(db=db)
        print(f"✓ Route handler check_duplicate_opportunities returned status '{dup_res['status']}'.")
