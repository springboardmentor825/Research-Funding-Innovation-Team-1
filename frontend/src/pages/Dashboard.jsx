...
function Panel({ title, subtitle, children, style }) {
  return (
    <div className="glass-card" style={{ padding: '1.5rem', backgroundColor: '#ffffff', ...style }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#f5f8ff' }}>
      ...
      <div style={{ padding: '0 2rem 2rem 2rem', display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '1200px', width: '100%', margin: '0 auto' }}>
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {analyticsError ? (
            <div className="glass-card" style={{ padding: '2rem', backgroundColor: '#ffffff', textAlign: 'center' }}>
                  ...
                  {analyticsLoading ? <p style={{ color: 'var(--text-secondary)' }}>Loading…</p> : (
                    <ChartShell height={300}>
                      <AreaChart data={trends} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                        ...
                        <YAxis tick={{ fill: '#64748b', fontSize: 12 }} tickFormatter={fmt} width={70} />
                        <Tooltip formatter={(value) => [fmt(value), 'Publications']} labelStyle={{ color: '#334155' }} contentStyle={{ borderRadius: 8, border: '1px solid #e2e8f0' }} />
                  ...
                  {analyticsLoading ? <p style={{ color: 'var(--text-secondary)' }}>Loading…</p> : (
                    <ChartShell height={280}>
                      <PieChart>
                        ...
                        <Tooltip formatter={(value, name) => [fmt(value), name]} contentStyle={{ borderRadius: 8, border: '1px solid #e2e8f0' }} />
                        <Legend verticalAlign="bottom" height={36} formatter={(value) => <span style={{ color: '#475569', fontSize: 12 }}>{value}</span>} />
                      </PieChart>
                    </ChartShell>
                  )}
                </Panel>
                  ...
                  {analyticsLoading ? <p style={{ color: 'var(--text-secondary)' }}>Loading…</p> : (
                    <ChartShell height={280}>
                      <BarChart data={topics} layout="vertical" margin={{ top: 5, right: 30, bottom: 5, left: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                        <XAxis type="number" tick={{ fill: '#64748b', fontSize: 12 }} tickFormatter={fmt} />
                        <YAxis type="category" dataKey="name" width={220} tick={{ fill: '#475569', fontSize: 11 }} tickFormatter={(v) => (v.length > 26 ? `${v.slice(0, 26)}…` : v)} />
                        <Tooltip formatter={(value) => [fmt(value), 'Publications']} contentStyle={{ borderRadius: 8, border: '1px solid #e2e8f0' }} />
                  ...
                  {analyticsLoading ? <p style={{ color: 'var(--text-secondary)' }}>Loading…</p> : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      {topCited.map((paper, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.75rem 1rem', backgroundColor: '#f8fafc', borderRadius: 10, border: '1px solid #eef2f7' }}>
